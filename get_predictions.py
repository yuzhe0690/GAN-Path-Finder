...
models = [
    define_G(channels, num_classes, img_size, 'batch', False, 'normal',
             0.02, gpu_id=device, use_ce=True, unet=False, attn=True)
]
...
for i, batch in enumerate(val_data_loader):
    if i > 5000:
        break
    input, target = batch[0].to(device), batch[1].to(
        device)  # .to(device), batch[1].to(device)
    predictions = []
    for num, model in enumerate(models):
        output = model(input)
        ppm = output[:, 0, ...]
        _, prediction = torch.max(output, 1, keepdim=False)
        ...
        # grid = np.round((ppm.mean(2) - ppm.mean(2).min()) /
        #                 ppm.mean(2).max(), 2)
        ppm_detached = ppm.detach().cpu()
        grid = np.round((ppm_detached.mean(2) - ppm_detached.mean(2).min()) /
                        ppm_detached.mean(2).max(), 2)

        grid[grid == 1] = 0.99
        if np.argwhere(input.mean(2) == 0).shape[0] < 2:
            continue
        # Convert input tensor to numpy and get coordinates
        input_np = input.mean(2).cpu().detach().numpy()
        zero_coords = np.argwhere(input_np == 0)

        # Handle cases where we might have more than 2 points
        if len(zero_coords) >= 2:
            # Take the first and last points as start and finish
            start_point = zero_coords[0]
            finish_point = zero_coords[-1]
            sx, sy = start_point
            fx, fy = finish_point
        else:
            # Handle the error case where we don't have enough points
            print(
                f"Warning: Found {len(zero_coords)} points with value 0, expected at least 2")
            continue  # Skip this iteration if we don't have enough points
...
with open('results.txt', 'w') as f:
    f.writelines(["steps focal = %s\n" % np.array(
        focal_astar[w]['steps']).mean() for w in [1, 1.5, 2, 5, 10]])
    f.writelines(["nodes focal = %s\n" % np.array(
        focal_astar[w]['nodes']).mean() for w in [1, 1.5, 2, 5, 10]])
    f.writelines(["length focal = %s\n" % np.array(
        focal_astar[w]['length']).mean() for w in [1, 1.5, 2, 5, 10]])
